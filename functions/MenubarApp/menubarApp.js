...
//
// Appends the comment (plus optional screenshot) to the default assistant's
// direct chat (objectType 'assistants') in the user's default project, then
// queues the assistant reply via ASSISTANT_RUNS_COLLECTION — the reply is
// generated by the processMenubarAssistantRun Firestore trigger, never inside
// this request (gen2 CPU throttling after res.send would starve it).
async function handleMenubarAssistantMessage(req, res) {
    ...
    try {
        ...
        try {
            ...
            // Ensure the assistant's direct chat exists (chatId === assistantId).
            const chatRef = db.doc(`chatObjects/${defaultProjectId}/chats/${assistantId}`)
            ...
            if (!chatDoc.exists) {
                await chatRef.set({
                    ...
                    lastEditorId: userId,
                    commentsData: {
                        lastCommentOwnerId: userId,
                        lastComment: preview,
                        lastCommentType: STAYWARD_COMMENT,
                        amount: 1,
                    },
                    hasStar: '#ffffff',
                    creatorId: actor.assistantData?.creatorId || userId,
            ...
            // Keep the chat navigable in the UI (best-effort, mirrors heartbeat topics).
            await Promise.all([
                db
                    .doc(`usersFollowing/${defaultProjectId}/entries/${userId}`)
                    .set({ assistants: { [assistantId]: true } }, { merge: true }),
                db
                    .doc(`followers/${defaultProjectId}/assistants/${assistantId}`)
                    .set({ usersFollowing: admin.firestore.FieldValue.arrayUnion(userId) }, { merge: true }),
            ]).catch(error => console.warn('menubarAssistantMessage: follower update failed', error))

            const commentId = db.collection('_').doc().id
            await db.doc(`chatComments/${defaultProjectId}/assistants/${assistantId}/comments/${commentId}`).set({
                commentText,
                mediaContext,
            ...
            const response = {
                ...
                chatId: assistantId,
                commentId,
                url: `${getAppBaseUrl()}/projects/${defaultProjectId}/assistants/${assistantId}/chat`,
            }

            await messageRef.set(
                { status: 'completed', response, commentId, chatId: assistantId, updatedAt: Date.now() },
                { merge: true }
            )
