...
const PINNED_CODEX_CLI_VERSION = '0.144.4'
...
function buildAgentCliInstallGuard(binaryName, packageName, targetVersion = 'latest', requiredVersion = null) {
    const packageSpec = `${packageName}@${targetVersion}`
    const usableCheck = requiredVersion
        ? `command -v ${binaryName} >/dev/null 2>&1 && ` +
          `installed_version="$(${binaryName} --version 2>/dev/null | head -n 1)" && ` +
          `installed_version="\${installed_version##* }" && [ "$installed_version" = "${requiredVersion}" ]`
        : `command -v ${binaryName} >/dev/null 2>&1`
    return (
        ...
        `npm install -g --prefix ${AGENT_CLI_NPM_PREFIX} ${packageSpec} || exit $?; ` +
        `printf 'AGENT_CLI_READY installed\\n'; fi`
    )
}

function buildClaudeInstallGuard() {
    return buildAgentCliInstallGuard('claude', '@anthropic-ai/claude-code')
}

function buildCodexInstallGuard() {
    return buildAgentCliInstallGuard('codex', '@openai/codex', PINNED_CODEX_CLI_VERSION, PINNED_CODEX_CLI_VERSION)
}
