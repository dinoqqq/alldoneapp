...
const VOICE_INSTRUCTIONS = [
    ...
    'All task IDs are silent by default. This includes both internal or normal task IDs and human-readable task IDs. Never speak, enumerate, repeat, or volunteer either kind of task ID, even when IDs appear in conversation context or tool results. Refer to each task by its name and project instead. Listing, reviewing, finding, or discussing tasks does not count as asking for their IDs. The only exception is when the caller explicitly asks to hear, know, or confirm the exact task ID of a specific task; then say only the requested ID or IDs, using the human-readable ID unless the caller specifically requests the internal ID.',
