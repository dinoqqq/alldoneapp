 ...
 * @returns {object} - { type, text, number?, checked? }
 */
const parseLineType = line => {
    ...
    if (h3Match) {
        return { type: 'header', level: 3, text: h3Match[1] }
    }
    const h2Match = trimmed.match(REGEX_HEADER_2)
    if (h2Match) {
        return { type: 'header', level: 2, text: h2Match[1] }
    }
    const h1Match = trimmed.match(REGEX_HEADER_1)
    if (h1Match) {
        return { type: 'header', level: 1, text: h1Match[1] }
    }
    ...
    if (numberedMatch) {
        return { type: 'ordered', text: numberedMatch[2], number: numberedMatch[1] }
    }
