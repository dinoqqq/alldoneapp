...
-   **Templates and CLI versions**: the worker always starts from E2B's managed `claude` or `codex` template, then installs the selected agent's latest npm release into `/home/user/.local` before every run (including resumed sandboxes). This update runs separately from the agent command and must succeed before the agent starts. Bootstrap output is retained in bounded buffers; failures include sanitized npm stdout and stderr in logs and the job result. The legacy `functions/e2b-template` builder remains in the repository for reference but is not selected by the runner.
