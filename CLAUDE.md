...
-   **Templates, CLI versions, and npm cache**: the worker always starts from E2B's managed `claude` or `codex` template, then installs the selected agent's latest npm release into `/home/user/.local` before every run (including resumed sandboxes). E2B's default `/home/user/.npm` can come from a read-only image layer, and Codex's nested workspace sandbox does not permit a replacement elsewhere in the home directory. `vmJobRunner.js` therefore centrally injects `NPM_CONFIG_CACHE=/tmp/alldone-npm-cache` into both the CLI bootstrap and the agent process. Repository `npm install` / `npm ci` commands inherit the writable per-sandbox cache on their first attempt; do not add command-specific `/tmp` retries. This VM-only environment leaves local development and CI npm configuration unchanged. The CLI update runs separately from the agent command and must succeed before the agent starts. Bootstrap output is retained in bounded buffers; failures include sanitized npm stdout and stderr in logs and the job result. The legacy `functions/e2b-template` builder remains in the repository for reference but is not selected by the runner.
