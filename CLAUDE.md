...
-   **Templates and CLI versions**: the worker always starts from E2B's managed `claude` or `codex` template. It uses the template's CLI when available and installs the matching `@latest` CLI into `/home/user/.local` only when the binary is missing. The legacy `functions/e2b-template` builder remains in the repository for reference but is not selected by the runner.
