...
-   **Concurrency and provider rate safety**: `functions/Assistant/vmJobConfig.js` is the shared source of truth for the five per-user admission slots and the five concurrent Cloud Tasks worker dispatches. A sixth in-flight job for the same user is rejected before Gold is charged or a task is enqueued; jobs waiting behind the five-worker global limit remain queued by Cloud Tasks. Worker starts are throttled to one per second so cold sandbox creation also stays within [E2B Hobby's documented limits](https://e2b.dev/docs/billing) (`1/sec` creation, 20 concurrent sandboxes). Deploy `runVmJob` after changing these values so Firebase updates the managed task queue.
...
-   **Templates and CLI versions**: the worker always starts from E2B's managed `claude` or `codex` template. Claude uses the template's CLI when available and installs `@anthropic-ai/claude-code@latest` only when `claude` is missing. Codex is centrally pinned by `PINNED_CODEX_CLI_VERSION` in `vmJobRunner.js` (currently `@openai/codex@0.144.4`, selected as the stable release used with the configured GPT-5.6 Sol model): it is installed or replaced in the logged bootstrap stage whenever `codex` is missing, its version cannot be read, or it differs from the pin. The legacy `functions/e2b-template` builder remains in the repository for reference but is not selected by the runner.
