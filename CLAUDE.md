...
-   **Context for the auto-created task** (`vmHostTaskHelper.js`): a fresh task has no thread conversation for `buildVmThreadContext` to read, so context is carried two ways. (1) The task is seeded **self-documenting**: `buildVmJobTaskName` sets a concise title from the objective, and `buildVmJobTaskDescription` fills the description with the objective + `**Deliverable:**` + `**Original request:**` (the original request line is omitted when it equals the objective), with any images the user attached embedded inline via `mergeTaskDescriptionWithImages`. This survives into later resumes and is what the user sees. (2) The VM agent's grounding bundle (`buildVmThreadContext`) takes an `originatingRequest` ({text, imageUrls}) — assembled in the dispatcher from `userContext.message` + `userContext.currentMessageImageUrls` — and renders an "Originating request" + image-URLs section to stand in for the missing conversation/attachments; it's passed only on the contextless path (in an existing thread the real conversation already covers it). Everything else in the bundle (user/project descriptions, user memory, assistant persona, date/time, language) resolves from docs and works for a brand-new task unchanged.
