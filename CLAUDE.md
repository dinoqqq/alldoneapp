...
-   **GitLab coding flow (clone → code → Merge Request)**: a `prototype` VM task can run inside a checkout of the project's connected GitLab repo and open an MR. Wiring: per-project repo config (`gitlabRepoUrl` / `gitlabBaseBranch` / `gitlabHost`) lives on the `projects/{projectId}` doc (non-secret, member-readable); the **per-user** token lives at `users/{uid}/private/gitlabAuth_{projectId}` (owner+server read only — never on the project doc). The `connectGitlabRepo` / `disconnectGitlabRepo` callables (`functions/Gitlab/gitlabConnect.js`, registered in `index.js`) validate the token + repo against the GitLab REST API and persist it; the UI is `components/ProjectDetailedView/ProjectProperties/ConnectGitLab/` (mirrors ConnectGmail, but a simple paste-token form). In `vmJobRunner.js`, `loadGitlabContext()` reads the repo + the **requesting user's** token, `setupGitRepo()` clones (fresh) or fetches (resume) into `/home/user/repo`, and `buildAgentPrompt()` instructs the agent to branch → commit → push with **GitLab push options** (`-o merge_request.create -o merge_request.target=<base> …`, no `glab`/API needed) and return the MR URL in its final message. **Security**: the token is injected ONLY as a per-command env var (`GITLAB_TOKEN`) and a git credential helper that resolves `$GITLAB_TOKEN` at push time — it is never written to `.git/config`, `prompt.txt`, logs, or the paused-session snapshot. Recommend users supply a repo-scoped **Project Access Token** (`api` + `write_repository`, Developer+) and keep the base branch protected so the agent can only open MRs, not push to it or merge.
